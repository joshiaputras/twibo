...
CREATE POLICY "Admins can view all campaigns" ON public.campaigns FOR SELECT TO authenticated USING (has_role(auth.uid(), 'admin'::app_role));
CREATE POLICY "Admins can update all campaigns" ON public.campaigns FOR UPDATE TO authenticated USING (has_role(auth.uid(), 'admin'::app_role)) WITH CHECK (has_role(auth.uid(), 'admin'::app_role));
CREATE POLICY "Admins can delete all campaigns" ON public.campaigns FOR DELETE TO authenticated USING (has_role(auth.uid(), 'admin'::app_role));
CREATE POLICY "Published campaigns are publicly readable" ON public.campaigns FOR SELECT USING (status = 'published'::text);
...
CREATE POLICY "Admins can view all campaign stats" ON public.campaign_stats FOR SELECT TO authenticated USING (has_role(auth.uid(), 'admin'::app_role));
