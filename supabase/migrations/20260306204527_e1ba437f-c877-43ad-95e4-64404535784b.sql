...
-- Fix ALL tables: change RESTRICTIVE policies to PERMISSIVE
...
-- ============ campaigns ============
DROP POLICY IF EXISTS "Anyone can read featured campaigns" ON public.campaigns;
...
AS PERMISSIVE FOR SELECT TO public USING (status = 'published'::text);
...
AS PERMISSIVE FOR UPDATE TO authenticated USING (has_role(auth.uid(), 'admin'::app_role)) WITH CHECK (has_role(auth.uid(), 'admin'::app_role));
...
USING (EXISTS (SELECT 1 FROM campaigns c WHERE c.id = campaign_stats.campaign_id AND c.status = 'published'::text));
...
USING (auth.uid() = user_id AND status = ANY (ARRAY['pending'::text, 'failed'::text]));
...
AS PERMISSIVE FOR SELECT TO public
USING (status = 'paid'::text AND midtrans_order_id IS NOT NULL);
