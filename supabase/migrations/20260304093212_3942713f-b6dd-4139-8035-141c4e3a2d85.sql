...
CREATE POLICY "Admins can view all campaigns" ON public.campaigns FOR SELECT TO authenticated USING (public.has_role(auth.uid(), 'admin'));
...
CREATE POLICY "Admins can update all campaigns" ON public.campaigns FOR UPDATE TO authenticated USING (public.has_role(auth.uid(), 'admin')) WITH CHECK (public.has_role(auth.uid(), 'admin'));
CREATE POLICY "Admins can delete all campaigns" ON public.campaigns FOR DELETE TO authenticated USING (public.has_role(auth.uid(), 'admin'));
...
CREATE POLICY "Owners can view own campaign stats" ON public.campaign_stats FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM campaigns c WHERE c.id = campaign_stats.campaign_id AND c.user_id = auth.uid()));
CREATE POLICY "Admins can view all campaign stats" ON public.campaign_stats FOR SELECT TO authenticated USING (public.has_role(auth.uid(), 'admin'));
...
CREATE POLICY "Owners can view own daily stats" ON public.campaign_stats_daily FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM campaigns c WHERE c.id = campaign_stats_daily.campaign_id AND c.user_id = auth.uid()));
CREATE POLICY "Admins can view all daily stats" ON public.campaign_stats_daily FOR SELECT TO authenticated USING (public.has_role(auth.uid(), 'admin'));
